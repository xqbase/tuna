...
class MulticastHandler implements ConnectionHandler {
	private OriginServer origin;
	private Iterable<? extends Connection> connections;

	MulticastHandler(OriginServer origin, Iterable<? extends Connection> connections) {
		this.origin = origin;
	...
	public void send(byte[] b, int off, int len) {
		...
		for (Connection conn : connections) {
			...
			VirtualHandler handler = origin.virtualMap.get(conn);
			if (handler == null) {
				continue;
			}
